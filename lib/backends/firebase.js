...
(function () {
  ...
  baseRef.auth(config.FIREBASE_SECRET, function (err, result) {
    if (err) {
      throw('Unable to authenticate to Firebase!', err);
    } else {
      // log authentication success
      console.log('Successfully authenticated on Firebase');
    }
  });
  ...
  FirebaseBackend.insert = function (path, value, priority, callback) {
    var ref = getRef(path);
